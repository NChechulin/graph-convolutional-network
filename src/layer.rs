...
impl<'a> ConvolutionLayer<'a> {
    ...
    pub fn apply(&self, feature_matrix: &DMatrix<f32>) -> DMatrix<f32> {
        self.laplacian * feature_matrix * self.weights.clone()
    }
}
...
impl SoftmaxLayer {
    pub fn softmax(
        row: &Matrix<
            ...
            Const<1_usize>,
            Dynamic,
            SliceStorage<'_, f32, Const<1_usize>, Dynamic, Const<1_usize>, Dynamic>,
        >,
    ) -> RowDVector<f32> {
        // FIXME
        ...
        RowDVector::from_fn(row.ncols(), |_, el| row[el].exp() / sum)
    }
